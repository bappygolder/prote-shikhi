...
You are reading this because you are **Gemini CLI**. Other AI tools on this project read sibling files:
...
| Task | Model | Model ID |
|---|---|---|
| Architecture decisions, hard bugs | Gemini 2.5 Pro (deep think) | `gemini-2.5-pro` |
| Feature implementation (default) | Gemini 2.5 Pro | `gemini-2.5-pro` |
| Small edits, quick lookups | Gemini 2.5 Flash | `gemini-2.5-flash` |

Switch with the Gemini CLI's `/model` command. If the user asks for "Opus" or "Sonnet" or "Haiku", they're naming Claude tiers — translate to the equivalent Gemini tier above.
...
> Skills are defined in Claude Code format. Gemini CLI does not auto-load Claude skills, but the role behaviour described in each `SKILL.md` is plain markdown — read the file directly when the user invokes a role.
